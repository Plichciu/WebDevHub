...
const FormContact = () => {
  ...
  const validateForm = (e: FormEvent) => {
    ...
    if (trimmedName.length < 2) {
      showAlertFn("Imię musi zawierać co najmniej 2 znaki");
      return;
    }

    if (!emailRegex.test(trimmedEmail)) {
      showAlertFn("Proszę podać poprawny adres email");
      return;
    }

    if (trimmedPhone && !phoneRegex.test(trimmedPhone)) {
      showAlertFn("Proszę podać poprawny numer telefonu");
      return;
    }

    if (trimmedMessage.length < 10) {
      showAlertFn("Wiadomość musi zawierać co najmniej 10 znaków");
      return;
    }
